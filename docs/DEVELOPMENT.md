...
Every status those events carry is an `AnalyzerStatus` member ([inspection_ledger.py](../src/skillspector/inspection_ledger.py)), never a bare string. Six values, of which `NON_LIMITING_STATUSES` holds the two that leave a Scan complete, and `analyzer_status_event` admits each status through the enum so a typo raises rather than surfacing as a mysterious limitation line. That runtime check is the enforcement, not the annotation: `mypy` is configured in this repository and invoked by nothing. `tests/unit/test_analyzer_status.py` reads the source and fails a status written inline — scoped to status *usage* rather than to the spelling, because `LedgerOutcome` shares `completed` and `failed` with it and writes them as bare strings on purpose.
